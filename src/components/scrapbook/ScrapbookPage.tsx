...
export default function ScrapbookPage() {
  ...
  }, [bookmarks, user, loaded]);

  const filtered = bookmarks.filter(b => {
    const matchSearch = !search
      || b.title.toLowerCase().includes(search.toLowerCase())
      || b.url.includes(search)
      || b.memo?.toLowerCase().includes(search.toLowerCase());
  ...
  const handleAdd = async () => {
    ...
    if (user) {
      await addBookmark(user.uid, { title: data.title, url: data.url, category: data.category, memo: data.memo });
    } else {
      setBookmarks(prev => [{ id: genId(), ...data }, ...prev]);
    }
  ...
  const handleDelete = (id: string) => {
    if (user) deleteBookmark(user.uid, id);
    ...
    <div style={{ maxWidth: "960px", margin: "0 auto" }}>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((b, i) => (
            ...
            >
              ...
              <span>
                <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: "10px", fontSize: "0.72rem", fontWeight: 600, background: getCategoryColor(b.category) + "18", color: getCategoryColor(b.category) }}>
                  {b.category || "기타"}
                </span>
              </span>
              <div style={{ minWidth: 0 }}>
                ...
                {b.url && (
                  <a href={ensureHttp(b.url)} target="_blank" rel="noopener noreferrer"
                    ...
                    <ExternalLink size={11} />{b.url.replace(/^https?:\/\//, "").split("/")[0]}
                  </a>
                )}
              </div>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)" }}
          ...
          <div style={{ width: "100%", maxWidth: "480px", borderRadius: "20px", padding: "24px", background: "var(--card)", border: "1px solid var(--border)", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {[
                { label: "제목 *", key: "title", placeholder: "제목 입력" },
                ...
                <div key={key}>
                  ...
                  <input value={form[key as keyof typeof form]} onChange={e => setForm({ ...form, [key]: e.target.value })}
                    placeholder={placeholder} autoFocus={key === "title"}
                  ...
                  {PRESET_CATEGORIES.map(cat => (
                    <button key={cat} onClick={() => setForm({ ...form, category: cat })}
                      style={{
                        ...
                        color: form.category === cat ? "#fff" : "var(--muted)", cursor: "pointer",
                      }}>
                      {cat}
                    </button>
                  ))}
